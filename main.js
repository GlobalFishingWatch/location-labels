...
try {
  ...
  const allLandFeatures = [...countriesJson.features, ...geonamesJson.features];
  ...
  const features = allLandFeatures.flatMap((feature) => {
    ...
    if (isCountry) {
      const { NAME_EN, NAME_ES, NAME_FR, NAME_PT, NAME_ID, NAME_ZH } =
        feature.properties;
      for (const char of [
        NAME_EN,
        NAME_ES,
  ...
  const seas = fs.readFileSync("./input/seas.geojson", "utf8");
  const seasJson = JSON.parse(seas);

  seasJson.features.forEach((feature) => {
    features.push({
      ...feature,
      tippecanoe: {
        minzoom: 0,
